              ...
              {syncing && (
                <div className="flex items-center gap-2">
                  <div className="h-2 w-28 overflow-hidden rounded-full bg-zinc-200">
                    <motion.div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-green-600" animate={{ width: `${syncProgress}%` }} transition={{ duration: 0.4 }} />
                  </div>
                    ...
                    <tbody className="divide-y divide-zinc-50">
                      {wellness.map((w) => {
                        const state = !w.hrv ? null
                          : w.hrv > 80 ? { label: d["state.optimal"], color: "text-green-600 bg-green-50" }
            ...
            {configured && existingCreds?.athleteId && (
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-5 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-green-800 text-sm">{d["connected.title"]}</p>
                ...
                <button onClick={handleDeleteCredentials}
                  className="text-green-600 hover:text-red-500 transition-colors p-2 rounded-lg hover:bg-red-50 flex-shrink-0"
                  title={d["disconnect"]}>
            ...
            <div className="bg-white rounded-2xl border border-zinc-200 overflow-hidden">
              ...
              <div className="bg-gradient-to-r from-zinc-900 to-zinc-700 px-6 py-5 text-white">
                ...
                <div className="flex items-center justify-center gap-2 mt-5 text-xs font-medium">
                  ...
                  <ChevronRight className="w-4 h-4 text-zinc-400" />
                  <span className="px-3 py-1.5 bg-green-500/90 rounded-lg flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> {d["wiz.thisApp"]}</span>
                </div>
              </div>
              ...
              <div className="p-6 space-y-0">
                ...
                <div className="relative pl-14">
                  <div className={`absolute left-0 top-0 w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm ${configured ? "bg-green-500 text-white" : "bg-zinc-900 text-white"}`}>
