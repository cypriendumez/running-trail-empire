...
export async function POST(req: NextRequest) {
  ...
  if (RESEND_API_KEY) {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Running & Trail Empire <noreply@running-trail-empire.com>",
        to: [to],
        subject,
        text: emailBody,
        html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          <div style="background:#09090b;border-radius:16px;padding:32px;color:white">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px">
              <div style="width:40px;height:40px;background:#18181b;border-radius:12px;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:18px">R</div>
              <span style="font-weight:600;font-size:16px">Running & Trail Empire</span>
            </div>
            <p style="color:#a1a1aa;font-size:14px;line-height:1.6;white-space:pre-wrap">${emailBody.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
            <hr style="border:none;border-top:1px solid #27272a;margin:24px 0" />
            <p style="color:#52525b;font-size:12px">Running & Trail Empire · Ne pas répondre à cet email</p>
          </div>
        </div>`,
      }),
    });
