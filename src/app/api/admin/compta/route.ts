...
async function admin() {
  ...
  return estAdmin(user?.email) ? user : null;
}
...
export async function GET() {
  ...
  const [ecr, reg] = await Promise.all([
    sb.from("notifications").select("id, user_id, data, created_at, title")
      .eq("user_id", user.id).eq("type", TYPE_ECRITURE).order("created_at", { ascending: false }).limit(5000),
    sb.from("notifications").select("data").eq("user_id", user.id).eq("type", TYPE_REGLAGES).maybeSingle(),
  ]);
...
export async function POST(req: Request) {
  ...
  const { data, error } = await createAdminClient().from("notifications").insert({
    user_id: user.id, type: TYPE_ECRITURE, title: ecriture.libelle, read: true,
...
export async function PATCH(req: Request) {
  ...
  if (b.action === "reglages") {
    ...
    const { data: exist } = await sb.from("notifications").select("id").eq("user_id", user.id).eq("type", TYPE_REGLAGES).maybeSingle();
    const { error } = exist
      ? await sb.from("notifications").update({ data: propre }).eq("id", exist.id)
      : await sb.from("notifications").insert({ user_id: user.id, type: TYPE_REGLAGES, title: "Réglages comptables", read: true, data: propre });
  ...
  if (b.action === "annuler") {
    ...
    const { data: ligne, error: eLire } = await sb.from("notifications")
      .select("id, user_id, data, created_at, title").eq("id", String(b.id ?? "")).eq("user_id", user.id).eq("type", TYPE_ECRITURE).maybeSingle();
