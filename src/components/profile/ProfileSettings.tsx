...
export function ProfileSettings({ profile, baseline, shoes, goals: initialGoals, stats, fitness, userId, etatAbo, catalogue = [] }: {
  ...
  stats: Stats;
  fitness?: { estimatedVma: number | null; vmaSource?: VmaSource; obsMaxHr: number | null; garminVo2max?: number | null;
   ...
   *  — alors que le coach a simplement jugé son volume insuffisant. */
  const [doubleEtat, setDoubleEtat] = useState<null | {
    autorise?: boolean; manque?: string[]; doubleSeuil?: boolean; manqueSeuil?: string[];
    volumeKm?: number; cetteSemaineKm?: number; error?: string;
  }>(null);
  ...
  // `estimatedVma` sort déjà de `effectiveVma` (test exclu) : la VO2max y est CROISÉE
  // avec la courbe d'allure, elle n'est plus un dernier recours qu'on n'atteignait jamais.
  const effVma = baselineVma > 0 ? baselineVma : (fitness?.estimatedVma ?? 0);
  const vmaSource: VmaSource = baselineVma > 0 ? "test" : (fitness?.vmaSource ?? null);
            ...
            <div className="bento-card space-y-4">
              ...
              <div className="col-span-2">
                ...
                <div className="col-span-2 rounded-2xl border border-zinc-200 bg-zinc-50/60 p-3">
                  <Toggle enabled={form.double_sessions} onToggle={async () => {
                    const actif = !form.double_sessions;
                    setForm(f => ({ ...f, double_sessions: actif }));
                    if (!actif) { setDoubleEtat(null); return; }
                    // Le verdict est demandé à l'ACTIVATION, pas à l'enregistrement :
                    // c'est au moment du geste que la question se pose.
                    setDoubleCheck(true);
                    try {
                      const r = await fetch("/api/coach/doubles");
                      setDoubleEtat(r.ok ? await r.json() : { error: "x" });
                  ...
                  {!doubleCheck && doubleEtat?.error && (
                    <p className="mt-2 text-[11px] text-zinc-500">{tr("double.checkFail")}</p>
                  )}
                  {!doubleCheck && doubleEtat && !doubleEtat.error && (
                    doubleEtat.autorise ? (
                      <div className="mt-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[12px] leading-relaxed text-emerald-900">
                        <p className="font-semibold">{tr("double.ok")}</p>
                        {doubleEtat.doubleSeuil === false && (doubleEtat.manqueSeuil ?? []).length > 0 && (
                          <p className="mt-1 text-emerald-800/80">{tr("double.noThreshold")} {doubleEtat.manqueSeuil!.join(" · ")}</p>
                        )}
                      </div>
                    ) : (
                      <div className="mt-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] leading-relaxed text-amber-900">
                        <p className="font-semibold">{tr("double.blocked")}</p>
                        <ul className="mt-1 space-y-0.5">
                          {(doubleEtat.manque ?? []).map((m, i) => <li key={i}>· {m}</li>)}
                        </ul>
                        <p className="mt-1.5 text-amber-800/80">{tr("double.kept")}</p>
                      </div>
                    )
                  )}
                </div>
        ...
        {tab === "subscription" && (
          <motion.div key="subscription" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="bento-card">
              ...
              {/* ⚠️ CE BANDEAU RÉPOND AUX TROIS QUESTIONS QU'UN ABONNÉ SE POSE, et
                  auxquelles l'écran ne répondait à AUCUNE : quand suis-je prélevé,
                  jusqu'à quand ai-je accès après avoir résilié, et mon paiement est-il
                  passé. L'ordre des cas est décidé par `messageAbo` : un échec de
                  paiement passe avant tout le reste, parce que c'est le seul cas où ne
                  rien faire coûte son abonnement à l'athlète. */}
              {(() => {
                const m = messageAbo(etatAbo);
                if (!m) return null;
                const quand = m.date
                  ? new Date(`${m.date}T00:00:00`).toLocaleDateString(lang === "fr" ? "fr-FR" : lang, { day: "numeric", month: "long", year: "numeric" })
                  : "";
                const urgent = m.cle === "echec";
                return (
                  <div className={`mb-4 rounded-2xl px-4 py-3 text-sm font-medium ${urgent ? "bg-red-50 text-red-800 ring-1 ring-inset ring-red-200" : "bg-emerald-50 text-emerald-900 ring-1 ring-inset ring-emerald-200"}`}>
                    {tr(`sub.${m.cle === "echec" ? "echec" : m.cle}`).replace("{d}", quand)}
                  </div>
                );
              })()}
            ...
            {profile?.subscription_tier === "free" && (
              <div className="bento-card bg-gradient-to-br from-zinc-900 to-zinc-800 text-white">
                ...
                {/* ⚠️ Ce bloc annonçait « 10 € /mois, ou 84 €/an (-30 %) » pour un palier
                    « Pro ». TROIS chiffres dont aucun n'existait : les formules sont
                    Starter et Premium, à 9,99 € et 14,99 €, et l'annuel vaut deux mois
                    offerts (-17 %), pas -30 %. Les montants viennent maintenant de
                    `lib/billing/prix`, la même source que l'accueil et /pricing. */}
                <div className="space-y-2 mb-4">
                  {(["starter", "premium"] as const).map((f) => (
                    <div key={f} className="flex items-baseline gap-2">
                      <span className="w-20 text-sm font-semibold capitalize text-white/80">{f}</span>
                      <span className="text-2xl font-black tabular-nums">{euros(PRIX_AFFICHES[f].mois, lang)}</span>
                      <span className="text-zinc-400 text-sm">{tr("sub.perMonth")}</span>
                      <span className="ml-auto text-xs text-emerald-400 font-semibold tabular-nums">
                        {tr("sub.yearly")
                          .replace("{an}", euros(PRIX_AFFICHES[f].an, lang))
                          .replace("{mois}", String(MOIS_OFFERTS))}
                      </span>
                    </div>
                  ))}
                </div>
                <a href="/pricing" className="block w-full text-center bg-emerald-500 hover:bg-emerald-400 text-white py-3 rounded-xl font-semibold transition-all">
                  {tr("sub.trial").replace("{n}", String(JOURS_ESSAI))} <ChevronRight className="w-4 h-4 inline" />
                </a>
              </div>
            )}
          </motion.div>
