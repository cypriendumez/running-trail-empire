...
export function ProfileSettings({ profile, baseline, shoes, goals: initialGoals, stats, fitness, userId }: {
  ...
  const [form, setForm] = useState({
    ...
    main_terrain: String(profile?.main_terrain ?? ""),
  ...
  async function save() {
    ...
    await supabase.from("profiles").update({
      ...
      main_terrain: form.main_terrain || null,   // la contrainte CHECK refuse la chaîne vide
          ...
          <motion.div key="profile" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="bento-card space-y-4">
              ...
              <div className="col-span-2">
                ...
                <div className="col-span-2">
                  <label className="text-xs font-medium text-zinc-500 block mb-1.5">{tr("f.terr")}</label>
                  <div className="grid grid-cols-3 gap-1.5 sm:grid-cols-6">
                    {([["plat", "f.terrPlat"], ["vallonne", "f.terrVallonne"], ["montagne", "f.terrMontagne"], ["plage", "f.terrPlage"], ["piste", "f.terrPiste"], ["mixte", "f.terrMixte"]] as const).map(([v, k]) => (
                      <button key={v} type="button" onClick={() => setForm(f => ({ ...f, main_terrain: v }))}
                        className={`py-2 rounded-xl text-xs font-medium border transition-all ${form.main_terrain === v ? "bg-zinc-900 text-white border-zinc-900" : "bg-white text-zinc-600 border-zinc-200 hover:bg-zinc-50"}`}>
                        {tr(k)}
                      </button>
                    ))}
                  </div>
