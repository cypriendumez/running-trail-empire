...
import { LifeBuoy, X, Send, ArrowRight, Zap } from "lucide-react";
...
const T: Record<string, Record<string, string>> = {
  fr: { title: "Aide", sub: "Une question sur l'app ? Je regarde ton compte.", ph: "Ex : mes séances n'arrivent pas sur ma montre",
    ...
    err: "Connexion impossible. Réessaie dans un instant.",  instant: "Réponse instantanée", online: "En ligne", suggest: "Questions fréquentes", open: "Ouvrir l'aide", close: "Fermer l'aide" },
  en: { title: "Help", sub: "A question about the app? I check your account.", ph: "E.g. my sessions don't reach my watch",
    ...
    err: "Connection failed. Try again in a moment.",  instant: "Instant answer", online: "Online", suggest: "Common questions", open: "Open help", close: "Close help" },
  de: { title: "Hilfe", sub: "Eine Frage zur App? Ich sehe mir dein Konto an.", ph: "Z. B. meine Einheiten kommen nicht auf die Uhr",
    ...
    err: "Verbindung fehlgeschlagen. Bitte gleich nochmal versuchen.",  instant: "Sofortantwort", online: "Online", suggest: "Häufige Fragen", open: "Hilfe öffnen", close: "Hilfe schließen" },
  es: { title: "Ayuda", sub: "¿Una duda sobre la app? Miro tu cuenta.", ph: "Ej.: mis sesiones no llegan al reloj",
    ...
    err: "Conexión imposible. Inténtalo en un momento.",  instant: "Respuesta instantánea", online: "En línea", suggest: "Preguntas frecuentes", open: "Abrir la ayuda", close: "Cerrar la ayuda" },
  pt: { title: "Ajuda", sub: "Uma dúvida sobre a app? Vejo a tua conta.", ph: "Ex.: as minhas sessões não chegam ao relógio",
    ...
    err: "Ligação impossível. Tenta daqui a pouco.",  instant: "Resposta instantânea", online: "Online", suggest: "Perguntas frequentes", open: "Abrir a ajuda", close: "Fechar a ajuda" },
};
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            <div className="flex items-center gap-3 border-b border-zinc-100 bg-gradient-to-b from-emerald-50/60 to-white px-4 py-3.5">
              <span className="relative flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-[0_4px_12px_-4px_rgba(5,150,105,0.6)]">
                <LifeBuoy className="h-5 w-5" />
                {/* Pastille « en ligne » : elle dit que l'aide répond MAINTENANT. Sans elle,
                    une fenêtre de chat ressemble à un formulaire de contact — on écrit et
                    on attend un e-mail. */}
                <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-white bg-emerald-400" />
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-[15px] font-semibold tracking-tight text-zinc-900">{t("title")}</span>
                  <span className="text-[10px] font-medium uppercase tracking-wide text-emerald-600">{t("online")}</span>
                </div>
                <div className="truncate text-xs text-zinc-500">{t("sub")}</div>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
              {shown.map((m, i) => (
                <div key={i} className={`flex flex-col ${m.role === "user" ? "items-end" : "items-start"}`}>
                  ...
                  {m.source && (
                    <span className="mt-1 inline-flex items-center gap-1 pl-1 text-[10px] font-medium text-emerald-600">
            ...
            <form onSubmit={(e) => { e.preventDefault(); send(input); }}
              className="flex items-center gap-2 border-t border-zinc-100 bg-zinc-50/60 px-3 py-3">
