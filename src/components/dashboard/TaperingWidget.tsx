...
export function TaperingWidget({ workouts, raceDate }: TaperingWidgetProps) {
  ...
  const tsbStatus =
    tsb >= 10 ? { label: "Affûtage optimal", color: "text-green-600", bg: "bg-[#D1FAE5]" }
    : tsb >= 0 ? { label: "Forme en hausse", color: "text-blue-600", bg: "bg-[#E0F2FE]" }
    : tsb >= -10 ? { label: "Charge équilibrée", color: "text-orange-500", bg: "bg-[#FFEDD5]" }
    : { label: "Surcharge — récupérez", color: "text-red-600", bg: "bg-red-50" };
    ...
    <div className="bg-white rounded-2xl border border-zinc-200 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-zinc-700">Charge & Affûtage</span>
        </div>
        {isTaperingWindow && (
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <MetricCard label="CTL" sublabel="Forme" value={Math.round(ctl)} color="text-blue-700" bg="bg-blue-50" />
        <MetricCard label="ATL" sublabel="Fatigue" value={Math.round(atl)} color="text-orange-700" bg="bg-orange-50" />
        <MetricCard
          label="TSB"
          sublabel="Fraîcheur"
      ...
      <div className={`rounded-xl px-4 py-3 mb-4 ${tsbStatus.bg}`}>
        <div className={`text-xs font-semibold ${tsbStatus.color}`}>{tsbStatus.label}</div>
        ...
        <div className="border border-zinc-200 rounded-xl px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5 text-xs text-zinc-500">
              ...
              TSB cible le jour J : <span className="font-bold text-zinc-700">+{targetTSB}</span>
            </div>
            <div className="text-xs text-zinc-500">
              Prévu : <span className={`font-bold ${predictedRaceTSB >= targetTSB ? "text-green-600" : "text-orange-500"}`}>
          ...
          {predictedRaceTSB < targetTSB - 5 && (
            <p className="mt-2 text-xs text-orange-600 flex items-center gap-1">
              ...
              Réduisez le volume de {Math.round((targetTSB - predictedRaceTSB) * 3)} min/jour
            </p>
          )}
        </div>
      )}
      ...
      {/* 7-day TSB sparkline */}
      <div className="mt-4">
        <div className="text-xs text-zinc-400 mb-2">TSB 7 jours</div>
